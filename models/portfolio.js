...
exports.selectArtBySeries = (series_id) => {
  return db
    .query(
      `SELECT art.art_id, art.stock_id, art.art_title, art.completion, art.custom_link, art.alt_text, art.three_word_description FROM art WHERE art.series_id = $1 ORDER BY art.completion ASC;`,
      [series_id]
    )
    .then(({ rows: items }) => {
      return items;
